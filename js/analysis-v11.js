...
// 🚀 페이지 로드 시 자동으로 Face-API 모델 로드
if (typeof faceapi !== 'undefined') {
    console.log('🎯 [v11.0] Face-API 라이브러리 감지 - 모델 로드 시작');
    loadFaceDetectionModels().then(() => {
        window.faceApiReady = true;
        console.log('✅ [v11.0] Face-API 초기화 완료');
    }).catch(error => {
        console.error('❌ [v11.0] Face-API 초기화 실패:', error);
        window.faceApiReady = false;
    });
} else {
    console.warn('⚠️ [v11.0] Face-API 라이브러리가 아직 로드되지 않았습니다. 나중에 자동 로드됩니다.');
    
    // Face-API 로드 대기 (최대 10초)
    let attempts = 0;
    const checkInterval = setInterval(() => {
        attempts++;
        if (typeof faceapi !== 'undefined') {
            console.log('✅ [v11.0] Face-API 라이브러리 로드 완료 - 모델 로드 시작');
            ...
            console.error('❌ [v11.0] Face-API 라이브러리 로드 타임아웃 (10초)');
            clearInterval(checkInterval);
        }
    }, 500);
}
